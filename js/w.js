...
var wHs = {
    ...
    LoadPlayer: function (t = !1) {
        ...
        if (undefined == name || "" == name) { this.StartTimer(t ? 1 : 3); return; }
        if (name == this.hs_name) { this.StartTimer(15), this.Set2Box(this.hs_data); return; }
        $("#btn_loadplayer").addClass("searching");
        var e = this.hs_url + "playerskills?n=" + encodeURIComponent(name);
        e += "&t=" + secondstamp(), $.ajax({
            type: "get",
            dataType: "json",
            url: e,
            success: function (e) {
    ...
    wL = {
        ...
        Init: function () {
            wL.is_loaded = !0, $("#setting-box").append("<select id='list-lang'></select>"), $.ajax({
                type: "get",
                dataType: "json",
                url: data_urls[Math.floor(data_urls.length * Math.random())] + "/languages",
                success: function (t) {
                    t.zh.name = "简体中文", t["zh-tw"].name = "繁體中文", wL.lang_supports = t, wL.BuildLangList();
        ...
        BuildFailMsg: function () {
            this.is_loaded = !1, $("#list-lang-button").remove(), $("#list-lang-menu").parent().remove();
            $("#list-lang").before("<div style='text-align:center;font-weight: bold;color:#666;'>Failed loading languages.</div>"), $("#list-lang").remove()
        },
        BuildLangList: function () {
            var t = "";
            $.each(wL.lang_supports, function (e, a) {
                (a["lang_" + e] || "en" == e) && (flag = wL.flag_url + a.flag, $.preload(flag), t += "<option value='" + e, t += "' data-class='avatar' data-style='background-image: url(&apos;", t += flag + "&apos;);'>" + a.name + "</option>")
                ...
                _renderItem: function (t, e) {
                    ...
                    return e.disabled && a.addClass("ui-state-disabled"), $("<span>", {
                        style: e.element.attr("data-style"),
                        class: "ui-icon " + e.element.attr("data-class")
                    }).appendTo(i), a.append(i).appendTo(t)
                },
                _renderButtonItem: function (t) {
                    var e = $("<span>", {
                        class: "ui-selectmenu-text"
                    });
                    return e.addClass("flag").html(t.label), $(".sflag").attr("style", t.element.data("style")), e
                },
    ...
    wDrops = {
        ...
        Item: {
            ...
            RedoHeader: function (t) {
                ...
                s.css("display", "inline-block"), h1 = $("<div class='blue-header'><div class='main'>" + e + "</div></div>"), s.before(h1), h1.append("<div class='sub'></div>"), y = "<div>" + wL.GetString("Expected Value", "w"), y += " = $" + a + "</div>", h1.find(".sub").append(y)
            }
        },
        Bywiki: {
            PreData: function (t) {
    ...
    wStateBox = {
        labels_attr: {
            ...
            hs_pro: {
                a: ["woodcutting", "forging", "mining", "wizardry", "fishing", "fletching", "cooking", "jewelry", "farming", "breeding", "alchemy", "carpentry"],
                min: 1,
                lang: "i"
            },
            hs_other: {
        ...
        AddVersion: function () {
            ...
            t += '   <span class="fas icon-info" title="' + wL.GetString("info-ver", "w"), t += '" data-k="info-ver@w@title"></span><div>', $(".list-state").append(t), $.ajax({
                type: "get",
                dataType: "json",
                url: "https://rpg-de.mo.ee/version.json?t=" + secondstamp(),
                success: function (t) {
            ...
            for (var ci in object_base) {
                if (
                    ...
                    for (var fi in object_base[ci].params.results) {
                        ...
                        var requires_one_from =
                            object_base[ci].params.results[fi].requires_one_from;
                        for (var ei in object_base[ci].params.results[fi].returns) {
                            var id = object_base[ci].params.results[fi].returns[ei].id;
                            var t = $.extend(
                                !0,
                                {},
                                object_base[ci].params.results[fi].returns[ei]
                            );
            ...
            $.each(SkillQuest.quests, function (k, v) {
                ...
                if (item_base[v.item_id].kcraft != undefined) {
                    for (var i in item_base[v.item_id].kcraft) {
                        if (
                            item_base[v.item_id].kcraft[i].skill_name != "health" &&
                            ...
                            if (v.skill == "breeding") {
                                ...
                                arr_lvl.push(
                                    Math.max(
                                        pets[n[0]].params.breeding_level,
                                        pets[n[1]].params.breeding_level
                                    )
                                );
                            } else {
                                arr_lvl.push(
                                    item_base[v.item_id].kcraft[i].level == undefined ||
...
function capitaliseFirstLetter(t) {
...
function sleep() {
    return new Promise(t => setTimeout(t, 50))
}

function timestamp() {
    return (new Date).getTime()
}
String.prototype.splice = function (t, e) {
